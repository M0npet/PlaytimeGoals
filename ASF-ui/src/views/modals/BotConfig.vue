<template>
  <main v-if="bot" class="main-container main-container--bot-config">
    ...
    <div v-else class="container">
      ...
      <PlaytimeGoals :bot="bot" :model="model"></PlaytimeGoals>
...
<script>
  ...
  import PlaytimeGoals from '../../components/Config/PlaytimeGoals.vue';
  ...
  export default {
    name: 'BotConfig',
    components: { ConfigEditor, PlaytimeGoals },
    data() {
      return {
        loading: false,
        saving: false,
    ...
    methods: {
      async loadConfig() {
        ...
        try {
          ...
          this.fields = Object.keys(fields)
            .filter(
              key =>
                key !== 'GamesPlayedWhileIdle' &&
                key !== 'CustomGamePlayedWhileIdle',
            )
            .map(key => {
      ...
      async onSave() {
        ...
        try {
          // PlaytimeGoals is the only owner of idle GamesPlayed.
          // Keep native ASF idling disabled to avoid competing writes.
          this.$set(this.model, 'GamesPlayedWhileIdle', []);
          this.$set(this.model, 'CustomGamePlayedWhileIdle', null);

          if (this.model.PlaytimeGoalsEnabled === true) {
            this.$set(
              this.model,
